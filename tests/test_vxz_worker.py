...
class BinaryContractTests(unittest.TestCase):
    def setUp(self):
        ...
        self.qef_rank = np.array([2, 3], dtype=np.uint8)
    ...
    def test_voxel_preview_header_and_records(self):
        ...
        self.assertEqual((magic, version, count, resolution), (b"VXVP", 3, 2, 8))
        ...
        np.testing.assert_array_equal(records["qef_rank"], self.qef_rank)
    ...
    def test_slice_payload_contains_only_matching_cells(self):
        ...
        np.testing.assert_array_equal(records["qef_rank"], self.qef_rank[:1])
    ...
    def test_mesh_payload_uses_float_positions_and_uint_indices(self):
        ...
        self.assertEqual((magic, version, vertex_count, index_count), (b"VXMP", 3, 3, 3))
    ...
    def test_voxel_payload_rejects_misaligned_qef_rank(self):
        with self.assertRaisesRegex(ValueError, "qef_rank must have shape"):
            vxz_worker.build_voxel_payload(
                self.coords,
                self.dual,
                self.flags,
